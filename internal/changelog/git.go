...
type Git struct {
	...
	Err error
	// NewestTag is the tag naming the highest version, empty where the
	// repository carries no version tag.
	NewestTag string
	// TaggedChangelog is the changelog as it stood at NewestTag, nil where
...
func (f *findings) git(c *Changelog, g *Git) {
	...
	if latest, ok := c.Latest(); ok && semver.Compare(latest.Version, canonical(g.NewestTag)) < 0 {
		f.add(CheckVersionBehindTag, latest.Line,
			"the newest entry is %s, which is behind the newest tag %s; an entry level with the tag is a release just made and one above it is a release pending",
			latest.Version[1:], g.NewestTag)
	}
	f.immutable(c, g)
}

// immutable reports released entries that have changed since the newest tag.
//
